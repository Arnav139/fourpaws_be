...
export const VaccinationRecord = (req: Request, res) => {
...
export const VaccinationSchedule = async (req : Request, res) => {
  const { petId } = req.query;
...
export const getAllPets = async (req: Request, res: Response) => {
  try {
    ...
    const DOG_BREEDS = [
      { breed: 'Labrador Retriever', traits: ['Friendly', 'Active', 'Outgoing'] },
      { breed: 'German Shepherd', traits: ['Loyal', 'Confident', 'Intelligent'] },
      { breed: 'Golden Retriever', traits: ['Gentle', 'Smart', 'Friendly'] },
      { breed: 'French Bulldog', traits: ['Playful', 'Adaptable', 'Charming'] },
      { breed: 'Siberian Husky', traits: ['Energetic', 'Independent', 'Friendly'] },
      { breed: 'Poodle', traits: ['Intelligent', 'Active', 'Elegant'] },
    ];
    ...
    const CAT_BREEDS = [
      ...
      { breed: 'Persian', traits: ['Gentle', 'Quiet', 'Sweet'] },
      { breed: 'Maine Coon', traits: ['Large', 'Gentle', 'Friendly'] },
      { breed: 'British Shorthair', traits: ['Calm', 'Patient', 'Easy-going'] },
      { breed: 'Bengal', traits: ['Active', 'Playful', 'Intelligent'] },
      { breed: 'Ragdoll', traits: ['Relaxed', 'Affectionate', 'Gentle'] },
    ];
    ...
    const BIRD_SPECIES = [
      ...
      { breed: 'Lovebird', traits: ['Social', 'Active', 'Affectionate'] },
      { breed: 'Canary', traits: ['Musical', 'Active', 'Gentle'] },
      { breed: 'African Grey', traits: ['Intelligent', 'Talkative', 'Social'] },
      { breed: 'Finch', traits: ['Social', 'Active', 'Musical'] },
    ];
    ...
    const getRandomWeight = (species: string, size: 'small' | 'medium' | 'large') => {
      const weightRanges = {
    ...
    const generateMockPets = (count: number = 20) => {
      ...
      for (let i = 0; i < count; i++) {
        ...
        const pet = {
          ...
          registrationNumber: Math.random() > 0.5 ? `REG${Math.floor(Math.random() * 10000)}` : undefined,
          bio: `A lovely ${breedInfo.breed} who is ${breedInfo.traits.join(', ').toLowerCase()}. Looking for a loving home!`,
          location: getRandomElement(LOCATIONS),
          photoUrl: `${getRandomElement(imageArray)}?w=500&q=80`,
          ownerID: `user-${Math.floor(Math.random() * 10) + 1}`,
          dateOfBirth: new Date(Date.now() - Math.random() * 31536000000 * 10).toISOString(),
        };
